...
<?php if (!empty($templates)): ?>
    <ul>
        <?php foreach ($templates as $key => $template) : ?>
            <li>
                <a href="<?php echo esc_url(admin_url('tools.php?page=' . MEKS_VIDEO_IMPORTER_PAGE_SLUG . '&template=') . $key ); ?>"><?php echo esc_html($template['name']); ?> (<?php echo esc_html($template['provider']); ?>) </a>
